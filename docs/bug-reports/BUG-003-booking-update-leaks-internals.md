...
The same internal leak occurs on `PUT /api/branding` when the payload fails validation — the identical Spring controller signature is exposed via `BrandingController.updateBranding`. This looks like a platform-wide default error handler rather than an endpoint-specific issue.
