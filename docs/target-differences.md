...
- **The exchange log doubled the slash on local paths — but the wire never did.** Because the local base URLs end with `/` and `RequestBuilder` paths start with one, the exchange logger, which built its URL by string-concatenating `baseURL + path`, recorded `http://localhost:3001/room//56`. That made 236 of 400 exchanges in a local run look wrong in the diagnostics. The **request itself was always correct**: axios resolves `baseURL + url` through its own `combineURLs`, which collapses the boundary slash, so the container received `/room/56` — verified against a raw echo server. The earlier worry that a stricter router would choke on `//` was therefore unfounded; nothing but our log ever saw it. Fixed in [#83](https://github.com/AKogut/restful-booker-api-testing/issues/83) by resolving the logged URL the same way axios resolves the request (`resolveUrl` in `http-client.ts`), so the log matches the wire. Live was never affected: its base URLs have no trailing slash.
